...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const videoWrap = document.querySelector("#video-container").offsetHeight;
  const thumbNail = document.querySelector("#thumbnail-grid").offsetHeight;
  const videoWrapHeight = window.innerHeight;
  const totalHeight = videoWrap + thumbNail + videoWrapHeight;
  document.body.style.height = `${totalHeight}`;
  console.log(totalHeight);
  gsap.set("#video-overlay", { clipPath: "circle(0% at 50% 50%)" });
  ...
  ScrollTrigger.create({
    ...
    onEnter: () => {
      // 각 썸네일 카드를 순차적으로 fade-in
      gsap.to(".portfolio-item", { autoAlpha: 1, duration: 1.5, stagger: 0.1 }); // duration과 stagger 값 조정
    },
    onLeaveBack: () => {
      // 각 썸네일 카드를 순차적으로 fade-out
